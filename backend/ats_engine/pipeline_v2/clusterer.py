"""
clusterer.py  (v3 — Unified Schema)
-------------------------------------
Zero-shot domain clustering of parsed resumes.

Expects parsed dicts in the unified schema from parser.py v4:
    parsed["info"]["name"]
    parsed["skills"]
    parsed["_meta"]["is_fresher"]
    parsed["_meta"]["raw_sections"]
...
def content_richness(parsed: dict) -> int:
    ...
    raw  = meta.get("raw_sections", {})
    score  = 0
    score += min(30, len(parsed.get("skills",     [])) * 5)
    score += min(20, len(parsed.get("experience", [])) * 10)
    score += min(20, len(parsed.get("projects",   [])) * 10)
    score += min(15, len(parsed.get("about", "").split()) // 5)
    score += min(10, len(parsed.get("achievements", [])) * 5)
    raw_words = sum(len(v.split()) for v in raw.values() if isinstance(v, str))
...
def _keyword_boost_scores(parsed: dict) -> dict:
    ...
    full_text_lower  = " ".join(
        v.lower() for v in parsed.get("_meta", {}).get("raw_sections", {}).values()
        if isinstance(v, str)
    )
    boosts: dict = {}
...
def classify_resume(parsed: dict, embeddings: dict, anchor_embeddings: dict) -> dict:
...
class ResumeClusterer:
    ...
    def __init__(self, model: SentenceTransformer, anchors: dict = None):
        ...
        status = "✅ rapidfuzz keyword boosting active" if RAPIDFUZZ_AVAILABLE else "⚠️  rapidfuzz not installed — pip install rapidfuzz"
        print(f"   {status}")
    ...
    def cluster(self, resume_db: dict) -> dict:
        ...
        for filename, data in resume_db.items():
            ...
            result["name"]       = parsed["info"]["name"]
            result["filename"]   = filename
            result["top_skills"] = parsed.get("skills", [])[:6]
            result["is_fresher"] = meta.get("is_fresher", False)
